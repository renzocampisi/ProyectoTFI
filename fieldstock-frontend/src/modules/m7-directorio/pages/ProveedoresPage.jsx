// src/modules/m7-directorio/pages/ProveedoresPage.jsx
// Hermana de TransportesPage / ClientesPage. Reusamos los mismos estilos
// (DirectorioPage.module.css) para mantener la familia visual coherente.
...
const CAMPOS = [
  ...
  { key: 'email',     label: 'Email',                 placeholder: 'ventas@empresa.com',               req: false },
  { key: 'contacto',  label: 'Contacto / Vendedor',   placeholder: 'Nombre del vendedor / referente',  req: false },
...
export default function ProveedoresPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try { setItems(await ProveedoresService.getAll({ q: busqueda || undefined })) }
  ...
  const handleSave = async (form) => {
    ...
    try {
      if (editItem) await ProveedoresService.update(editItem.id, form)
      else          await ProveedoresService.create(form)
  ...
  const handleDelete = async () => {
    ...
    try { await ProveedoresService.delete(confirmDel.id); setConfirmDel(null); await fetchItems() }
    catch (err) { setError(err.message) }
  }
  ...
  return (
    <div className={styles.page}>
      {showForm && (
        <FormModal
          titulo={editItem ? 'Editar proveedor' : 'Nuevo proveedor'}
      ...
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Empresas que venden herramientas o materiales al galpón</p>
        </div>
      ...
      {loading
        ...
        : items.length === 0
          ? <div className={styles.empty}>
              ...
              <button className={styles.btnPrimary} onClick={() => setShowForm(true)}>Agregar primer proveedor</button>
            </div>
          : <div className={styles.tableWrapper}>
              <table className={styles.table}>
                <thead><tr>
                  <th>Nombre</th><th>Rubro</th><th>Teléfono</th><th>Dirección</th>
