...
export default function ClientesPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try { setItems(await ClientesService.getAll({ q: busqueda || undefined })) }
  ...
  const handleSave = async (form) => {
    ...
    try {
      if (editItem) await ClientesService.update(editItem.id, form)
  ...
  const handleDelete = async () => {
    ...
    try { await ClientesService.delete(confirmDel.id); setConfirmDel(null); await fetchItems() }
    catch (err) { setError(err.message) }
  }
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Empresas y clientes asociados a las obras</p>
        </div>
